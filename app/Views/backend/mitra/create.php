...
<div class="card border-0 shadow-sm rounded-3">
    <div class="card-body p-4">
        ...
        <form action="<?= base_url('panel/mitra/store') ?>" method="post">
            <?= csrf_field() ?>

            <div class="row">
                <div class="col-md-12 mb-3">
                    <label for="nama" class="form-label fw-semibold">Nama Instansi / Perusahaan <span class="text-danger">*</span></label>
                    <input type="text" class="form-control <?= isset($errors['nama']) ? 'is-invalid' : '' ?>" id="nama" name="nama" value="<?= old('nama') ?>" placeholder="Contoh: PT. Telkom Indonesia">
                    <div class="invalid-feedback"><?= $errors['nama'] ?? '' ?></div>
                </div>

                <div class="col-md-12 mb-4">
                    <label for="image_input" class="form-label fw-semibold">Logo Mitra <span class="text-danger">*</span></label>
                    <input type="file" class="form-control <?= isset($errors['logo_base64']) ? 'is-invalid' : '' ?>" id="image_input" accept="image/*" required>
                    <div class="invalid-feedback"><?= $errors['logo_base64'] ?? '' ?></div>
                    <small class="text-muted">Logo akan otomatis dikompresi menjadi WebP dengan resolusi maksimal 500px.</small>
            ...
            <div class="text-end">
                <button type="submit" id="btnSubmit" class="btn btn-primary" disabled><i class="bi bi-save me-1"></i> Simpan Mitra</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    // Script kompresi sama persis seperti sebelumnya
