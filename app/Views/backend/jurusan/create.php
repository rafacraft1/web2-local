...
<div class="card border-0 shadow-sm rounded-3">
    <div class="card-body p-4">
        ...
        <form action="<?= base_url('panel/jurusan/store') ?>" method="post">
            <?= csrf_field() ?>

            <div class="row">
                <div class="col-md-8 mb-3">
                    <label for="name" class="form-label fw-semibold">Nama Jurusan <span class="text-danger">*</span></label>
                    <input type="text" class="form-control <?= isset($errors['name']) ? 'is-invalid' : '' ?>" id="name" name="name" value="<?= old('name') ?>" placeholder="Contoh: Rekayasa Perangkat Lunak">
                    <div class="invalid-feedback"><?= $errors['name'] ?? '' ?></div>
                </div>

                <div class="col-md-4 mb-3">
                    <label for="icon" class="form-label fw-semibold">Ikon (Emoji) <span class="text-danger">*</span></label>
                    <input type="text" class="form-control <?= isset($errors['icon']) ? 'is-invalid' : '' ?>" id="icon" name="icon" value="<?= old('icon') ?>" placeholder="Contoh: 💻 atau 🎨">
                ...
                <div class="col-12 mb-3">
                    ...
                    <input type="text" class="form-control <?= isset($errors['short_desc']) ? 'is-invalid' : '' ?>" id="short_desc" name="short_desc" value="<?= old('short_desc') ?>" placeholder="Muncul di kotak (card) halaman depan. Maksimal 255 karakter.">
                ...
                <div class="col-12 mb-4">
                    ...
                    <textarea class="form-control <?= isset($errors['description']) ? 'is-invalid' : '' ?>" id="description" name="description" rows="5"><?= old('description') ?></textarea>
                    <div class="invalid-feedback"><?= $errors['description'] ?? '' ?></div>
                </div>

                <div class="col-12 mb-4">
                    <label for="image_input" class="form-label fw-semibold">Gambar / Ilustrasi Jurusan <span class="text-danger">*</span></label>
            ...
            <div class="text-end">
                <button type="submit" id="btnSubmit" class="btn btn-primary" disabled><i class="bi bi-save me-1"></i> Simpan Jurusan</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    document.getElementById('image_input').addEventListener('change', function(event) {
        ...
        btnSubmit.disabled = true; // Disable tombol submit selama proses
        ...
        reader.onload = function(e) {
            ...
            img.onload = function() {
                ...
                // Resize maksimal lebar/tinggi 1024px agar proses kompresi lebih maksimal
                const MAX_SIZE = 1024;
                let width = img.width;
                ...
                // Rumus ukuran base64: Target Max 60KB (61440 Bytes). 
                // Panjang string base64 kira-kira 1.33x ukuran file asli, jadi batasnya sekitar 81.000 karakter string
                let maxLength = 81000;

                // Loop untuk menurunkan kualitas gambar jika ukuran masih di atas target
