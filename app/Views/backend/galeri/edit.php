...
<div class="card border-0 shadow-sm rounded-3">
    <div class="card-body p-4">
        ...
        <form action="<?= base_url('panel/galeri/update/' . $safeId) ?>" method="post">
            <?= csrf_field() ?>

            <div class="row">
                <div class="col-md-12 mb-3">
                    <label for="title" class="form-label fw-semibold">Judul Foto <span class="text-danger">*</span></label>
                    <input type="text" class="form-control <?= isset($errors['title']) ? 'is-invalid' : '' ?>" id="title" name="title" value="<?= old('title', $galeri['title']) ?>">
                    <div class="invalid-feedback"><?= $errors['title'] ?? '' ?></div>
                </div>

                <div class="col-md-12 mb-3">
                    <label for="description" class="form-label fw-semibold">Deskripsi Singkat <span class="text-danger">*</span></label>
                    <textarea class="form-control <?= isset($errors['description']) ? 'is-invalid' : '' ?>" id="description" name="description" rows="3"><?= old('description', $galeri['description']) ?></textarea>
                    <div class="invalid-feedback"><?= $errors['description'] ?? '' ?></div>
                </div>

                <div class="col-md-12 mb-4">
                    <label class="form-label fw-semibold d-block">Foto Saat Ini</label>
                    <img src="<?= base_url('uploads/galeri/' . $galeri['image']) ?>" alt="Foto Galeri" class="img-thumbnail mb-3" width="300">

                    <label for="image_input" class="form-label fw-semibold d-block">Ganti Foto <span class="text-muted fw-normal">(Opsional)</span></label>
